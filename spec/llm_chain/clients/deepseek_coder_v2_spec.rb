require 'spec_helper'
require 'llm_chain/clients/deepseek_coder_v2'

RSpec.describe LLMChain::Clients::DeepseekCoderV2 do
  ...
  describe '#initialize' do
    ...
    it 'allows overriding model and base_url' do
      client = described_class.new(model: 'deepseek-coder-v2:16b', base_url: 'http://foo')
      expect(client.instance_variable_get(:@model)).to eq('deepseek-coder-v2:16b')
    ...
    it 'merges custom options with defaults' do
      ...
      expect(opts[:top_p]).to eq(0.95)
    end

    it 'calls super with correct arguments' do
      expect_any_instance_of(LLMChain::Clients::OllamaBase).to receive(:initialize).with(hash_including(model: default_model, default_options: hash_including(temperature: 0.1)))
      described_class.new
    end
  end
end 